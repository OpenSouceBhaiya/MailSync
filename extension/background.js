...
chrome.alarms.onAlarm.addListener(async (alarm) => {
    ...
    try {
        ...
        // CRITICAL FIX: Do NOT call startListening() here — it would destroy the offscreen SSE doc.
        // Instead: only restart if the offscreen doc has crashed. Otherwise just do a safety poll.
        const hasDoc = await chrome.offscreen.hasDocument().catch(() => false);
        if (!hasDoc) {
            // Offscreen doc crashed — SSE is dead — full restart
            await startListening(data.uuid, data.aesKey, true);
        } else {
            // SSE is alive and healthy — just do a quick fetch in case SSE missed something
...
// Track in-flight OTP fetch to prevent concurrent duplicate fetches
