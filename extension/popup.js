...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Wire up error copy button — fixed: reads from storage and builds text correctly
        ...
        copyErrBtn.addEventListener('click', () => {
            chrome.storage.local.get(['globalErrors'], (data) => {
                ...
                if (errors.length === 0) {
                    ...
                    setTimeout(() => { copyErrBtn.textContent = '📋 Copy Errors'; }, 2000);
                    return;
                }
        ...
        if (btn) {
            btn.addEventListener('click', () => {
                ...
                confirmBtn.onclick = async () => {
                    ...
                    if (data.uuid) {
                        try {
                            await fetch(`https://mailsync-relay-default-rtdb.asia-southeast1.firebasedatabase.app/devices/${data.uuid}.json`, {
                                method: 'PATCH',
                    ...
                    // Show terminated view directly — no reload
                    showTerminatedView();
                };
            });
        }
    ...
    reloginBtn.addEventListener('click', async () => {
        chrome.runtime.sendMessage({ action: "stop_listening" }).catch(() => {});
        // Clear everything including terminated flag so QR flow starts fresh
        await chrome.storage.local.clear();
        startQRFlow({});
    });
    ...
    async function checkDeviceStatus(uuid) {
        try {
            const res = await fetch(`https://mailsync-relay-default-rtdb.asia-southeast1.firebasedatabase.app/devices/${uuid}.json?_t=${Date.now()}`);
    ...
    function applyStatusData(data) {
        ...
        if (data.status === 'error_no_accounts') {
            hideAll();
            document.getElementById('no-accounts-view').classList.remove('hidden');
            renderGreeting(nameToShow);
            if (statusIndicator) statusIndicator.className = 'status-dot-container failing';
            if (statusText) statusText.textContent = "No Accounts";
    ...
    function pollForLink(uuid, aesKey) {
        const firebaseUrl = `https://mailsync-relay-default-rtdb.asia-southeast1.firebasedatabase.app/devices/${uuid}.json`;
