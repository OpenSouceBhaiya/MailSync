...
object ErrorReporter {
    fun reportError(context: Context, e: Exception, tag: String = "MailSyncError") {
        ...
        com.mailsync.app.utils.ToastManager.show(context, "Error copied. Please report this bug.", android.widget.Toast.LENGTH_LONG)
    }
    ...
    fun reportApiException(context: Context, statusCode: Int, tag: String = "MailSyncError") {
        ...
        when (statusCode) {
            12501 -> {
                com.mailsync.app.utils.ToastManager.show(context, "Sign-in cancelled. Please select a Google Account to continue.", android.widget.Toast.LENGTH_LONG)
                return
            }
            7 -> {
                // NETWORK_ERROR: Google Play Services lost connection mid-handshake
                com.mailsync.app.utils.ToastManager.show(context, "Connection lost during sign-in. Please check your internet and try again.", android.widget.Toast.LENGTH_LONG)
                Log.e(tag, "Google Sign-In NETWORK_ERROR (status 7)")
                return
            }
            12500 -> {
                com.mailsync.app.utils.ToastManager.show(context, "Sign-in failed. Please try again.", android.widget.Toast.LENGTH_LONG)
