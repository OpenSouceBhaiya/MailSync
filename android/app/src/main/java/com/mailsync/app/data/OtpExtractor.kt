...
object OtpExtractor {
    ...
    private val metadataPrefixes = listOf(
        "src", "reference", "ref", "ticket", "case", "id", "order", "invoice", "tracking", "call", "phone", "contact",
        "upi", "rs", "rs.", "inr", "transaction", "txn", "sent", "received", "amount", "date"
    )
    ...
    fun extractOtp(subject: String?, bodyText: String?, bodyHtml: String?, receivedTimeMs: Long): ExtractionResult? {
        ...
        var result = extractFromText(truncatedBody, receivedTimeMs)
    ...
    private fun extractFromText(text: String, receivedTimeMs: Long): ExtractionResult? {
        ...
        // STRICT FILTER: If the email doesn't contain a single OTP-related keyword, it's not an OTP email.
        val hasOtpKeyword = triggerKeywords.any { normalized.contains(it, ignoreCase = true) }
        if (!hasOtpKeyword) {
            safeLog("OtpExtractor", "Rejected: No OTP keywords found in text.")
            return null
        }
        
        val numberRegex = Regex("\\b([0-9]{4,8}|[A-Z0-9]{4,8})\\b")
        ...
        for (match in matches) {
            ...
            // 1. Reject pure letters or non-digits
            if (!candidate.any { it.isDigit() } || candidate.all { it.isLetter() }) continue
            ...
            if (hasMetadataPrefix(normalized, startPos)) {
                score -= 1000 // Strongly penalize IDs, references, orders
            }
