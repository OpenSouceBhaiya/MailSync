                    ...
                    try {
                        ...
                        if (accessToken != null) {
                            if (settingsManager.isBackendSyncEnabled()) {
                                ...
                                Log.d("OtpRepository", "Backend sync disabled for $email — relying on push notifications")
                            }
                        } else {
                        ...
                        if (newOtps.isNotEmpty()) {
                            ...
                            AppDatabase.insertMutex.withLock {
                                newOtps.forEach { entity ->
                                    // Use a 6-minute window to catch duplicates between notification and Gmail API paths
                                    val existingByCode = otpDao.getOtpByCodeRecent(entity.code, System.currentTimeMillis() - 6 * 60 * 1000)
                                    ...
                                    if (existingByCode != null) {
                                        ...
                                        // Safe to insert new
                                        val entityWithExpiry = if (entity.expiresAt == null) {
                                            entity.copy(expiresAt = entity.receivedAt + 10 * 60 * 1000L)
                                        } else entity
                                        otpDao.insertOtp(entityWithExpiry)
                                        localFetchedOtps.add(entityWithExpiry)
                                        ...
                                        if (isRecent) {
                                            otpsToBroadcast.add(entityWithExpiry)
                                            otpsToClipboard.add(entityWithExpiry)
                                        }
                                    }
                                }
                            }
                            ...
                            for (entity in otpsToBroadcast) {
                                // Launch all broadcasts in parallel to minimize delay
                                    ...
                                    keys.forEach { (uuid, keyBase64) ->
                                        launch {
                                            firebaseManager.broadcastOtp(
                                                ...
                                                entity.expiresAt ?: (entity.receivedAt + 10 * 60 * 1000L)
                                            )
                                        }
                                    }
                                    if (keys.isNotEmpty()) {
                                        Log.d("OtpRepository", "Broadcasted OTP ${entity.code} to ${keys.size} PCs in parallel. Phone clipboard will ALWAYS copy as well.")
                                    }
                                }
                            }
